...
class Insert extends React.Component {
    ...
    async _fetchData() {
        ...
        const last_code = await productgroup_model.generateProductGroupLastCode({
            code: `PGC${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, "0")}`,
            digit: 3,
        })
    ...
    render() {
        ...
        return (
            <div>
                ...
                <Card>
                    <CardHeader>
                        <h3 className="text-header">เพิ่มกลุ่มเครื่องมือ / Add Product Group</h3>
                    </CardHeader>
                    <Form onSubmit={(event) => this._handleSubmit(event)}>
                        <CardBody>
                            <Row>
                                <Col md={4}>
                                    <label>รหัสกลุ่มเครื่องมือ <font color="#F00"><b>*</b></font></label>
                                    <Input
                                        type="text"
                                        value={this.state.product_group_code}
                                        readOnly
                                        onChange={(e) => this.setState({ product_group_code: e.target.value })}
                                        placeholder="รหัสกลุ่มเครื่องมือ"
                                    />
                                    <p className="text-muted">Example : </p>
                                </Col>
                                <Col md={4}>
                                    <FormGroup>
                                        <label>ชื่อกลุ่มเครื่องมือ <font color="#F00"><b>*</b></font></label>
                                        <Input
                                            type="text"
                                            value={this.state.product_group_name}
                                            // onChange={(e) => this.setState({ product_group_name: e.target.value })}
                                            placeholder="ชื่อกลุ่มเครื่องมือ"
                                            onClick={() => this.setState({
                                                show_modal: true,
                                                title_modal: 'ชื่อกลุ่มเครื่องมือ',
